...
async def home(request: Request, db: Session = Depends(get_db)):
    ...
    """
    recent_analyses = repository_service.get_history(db, limit=5, offset=0)
...
async def analyze(request: Request, url: str = Form(...), db: Session = Depends(get_db)):
    """
...
async def results(request: Request, analysis_id: int, db: Session = Depends(get_db)):
    ...
    """
    analysis = repository_service.get_analysis_by_id(db, analysis_id)
    if not analysis:
...
async def history(
    ...
    """
    analyses = repository_service.get_history(db, limit=limit, offset=offset)
