...
@app.command()
def analyze(
    url: str = typer.Argument(..., help="The public GitHub repository URL to analyze (e.g., https://github.com/fastapi/fastapi)"),
    profile: str = typer.Option("library", "--profile", "-p", help="Repository profile type: 'library', 'personal', or 'enterprise'"),
):
    """
    Run an engineering analysis on a GitHub repository.
    """
    console.print(f"[bold cyan]ArchLens CLI[/bold cyan] » Analyzing [bold]{url}[/bold] (Profile: {profile})")
    
    # Run the analysis with a spinning progress indicator
    with console.status("[bold green]Fetching repository data and running analysis...", spinner="dots"):
        try:
            result = asyncio.run(async_analyze(url, profile))
        except InvalidRepositoryURLError:
            console.print(f"[bold red]Error:[/bold red] Invalid GitHub repository URL provided: {url}")
            raise typer.Exit(code=1)
        except RepositoryNotFoundError as e:
            console.print(f"[bold red]Error:[/bold red] {str(e)}")
            raise typer.Exit(code=1)
        except RateLimitExceededError as e:
            console.print(f"[bold red]Error:[/bold red] GitHub API rate limit exceeded. Reset time: {e.reset_time}")
            raise typer.Exit(code=1)
