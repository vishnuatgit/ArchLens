...
class MetricsService:
    ...
    def calculate_documentation_score(
        self, root_contents: List[Dict[str, Any]]
        ...
        if has_readme:
            score += 10
            strengths.append(
                "Found repository README detailing project architecture and setup."
            )
        else:
            weaknesses.append("Missing README file in root directory.")
            suggestions.append(
                "Add a README.md file in the root to introduce your project to developers and SREs."
            )
        ...
        if has_license:
            ...
            suggestions.append(
                "Add a LICENSE file (e.g. MIT, Apache 2.0) to clarify code usage permissions."
            )
        ...
        if has_contributing or has_docs_dir:
            ...
            strengths.append(
                f"Found repository docs resource: {' and '.join(resources)}."
            )
        else:
            weaknesses.append(
                "Missing CONTRIBUTING guide and dedicated docs directory."
            )
            suggestions.append(
                "Create a CONTRIBUTING.md file or a docs/ folder to guide engineers on contribution workflows."
            )

        return score, strengths, weaknesses, suggestions

    def calculate_activity_score(
        self, metadata: Dict[str, Any], recent_commits: List[Dict[str, Any]]
    ) -> Tuple[int, List[str], List[str], List[str]]:
        """
        ...
        if commit_count >= 10:
            ...
            score += 2
            strengths.append(
                f"Low commit activity: Only {commit_count} commit(s) in the last 30 days."
            )
        else:
            weaknesses.append("Zero commit activity recorded in the last 30 days.")
            suggestions.append(
                "Establish a consistent release cycle and commit updates periodically to keep the repository active."
            )
        ...
        if days_since_push <= 30:
            ...
            suggestions.append(
                "Resume repository updates or push commits to demonstrate project lifecycle maintenance."
            )
    ...
    def calculate_organization_score(
        ...
        if has_tests:
            score += 8
            strengths.append(
                "Found dedicated test suite directory for verifying software stability."
            )
        else:
            weaknesses.append(
                "Missing test suite folder (e.g. tests/, test/) in the root directory."
            )
            suggestions.append(
                "Create a tests/ directory in your root and write automated unit tests for key logic."
            )
        ...
        if has_src:
            ...
            weaknesses.append(
                "Missing standard source code folder (e.g. src/ or app/) in root."
            )
            suggestions.append(
                "Structure your project by moving code files into an app/ or src/ directory instead of leaving them flat in the root."
            )
        ...
        if has_configs:
            score += 6
            strengths.append(
                "Found workspace configuration files (e.g. settings, packages, dependency lock files)."
            )
        else:
            weaknesses.append(
                "Missing standard environment configuration or dependency management files."
            )
            suggestions.append(
                "Add dependency manifest files (e.g., requirements.txt, pyproject.toml) to standardize workspace configurations."
            )

        return score, strengths, weaknesses, suggestions

    def calculate_community_score(
        self, metadata: Dict[str, Any], contributor_count: int
    ) -> Tuple[int, List[str], List[str], List[str]]:
        """
        ...
        if stars >= 50:
            ...
            suggestions.append(
                "Promote your repository on developer communities to attract more visibility."
            )
        ...
        if forks >= 20:
            score += 6
            strengths.append(
                f"High fork count ({forks}) indicates active reuse and collaboration."
            )
        elif forks >= 5:
            score += 3
            ...
            weaknesses.append(
                f"Low fork count ({forks}) suggests limited external collaboration."
            )
            suggestions.append(
                "Make the project easier to fork and contribute to by improving documentation."
            )
        ...
        if contributor_count >= 5:
            ...
            score += 4
            strengths.append(
                f"Small but active contributor group ({contributor_count} contributors)."
            )
        else:
            weaknesses.append(
                "Only a single contributor — repository lacks collaboration."
            )
            suggestions.append(
                "Open issues for good first contributions and invite collaborators."
            )

        return score, strengths, weaknesses, suggestions

    def calculate_maintainability_score(
        self, metadata: Dict[str, Any], workflow_contents: List[Dict[str, Any]]
    ) -> Tuple[int, List[str], List[str], List[str]]:
        """
        ...
        if has_workflows:
            score += 8
            strengths.append(
                f"Found {len(workflow_contents)} GitHub Actions workflow(s) for automated CI/CD."
            )
        else:
            weaknesses.append(
            ...
            suggestions.append(
                "Add a GitHub Actions workflow to automate testing and linting on every push."
            )
        ...
        if open_issues == 0:
            ...
            weaknesses.append(
                f"High open issue count ({open_issues}) may indicate maintenance debt."
            )
            suggestions.append(
                "Triage and close stale open issues to demonstrate active maintenance."
            )
        ...
        if repo_size_kb <= 50000:  # up to ~50MB
            ...
            weaknesses.append(
                f"Repository is large ({repo_size_kb // 1024}MB) — consider cleaning up binaries or large assets."
            )
            suggestions.append(
                "Use .gitignore to exclude build artifacts, binaries, and large generated files."
            )
